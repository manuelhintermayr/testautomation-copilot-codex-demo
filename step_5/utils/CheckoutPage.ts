...
export class CheckoutPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.deliveryMethodLabel = page.locator('label:has-text("Home Delivery")');
  ...
  /**
   * Check if pickup is selected (has blue border)
   */
  async isPickupSelected(): Promise<boolean> {
    const classes = await this.pickupMethodLabel.getAttribute('class');
    return classes?.includes('border-blue-600') || false;
  }

  /**
   * Check if delivery is selected (has blue border)
   */
