...
export class HomePage extends BasePage {
  ...
  async isProductsSectionInViewport(): Promise<boolean> {
    // Use bounding box to check if element is in viewport
    const boundingBox = await this.productsSection.boundingBox();
    if (!boundingBox) return false;
    
    const viewportSize = this.page.viewportSize();
    if (!viewportSize) return false;
    
    return boundingBox.y < viewportSize.height && boundingBox.y + boundingBox.height > 0;
  }
